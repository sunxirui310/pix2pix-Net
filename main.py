...
parser.add_argument('--phase', dest='phase', default='train', help='train, test, blind')
...
parser.add_argument('--fine_rot',dest='fine_rot',type=bool, default=True,help='if True :rotate train image to random angles')
...
#the two below work only when phase==blind
parser.add_argument('--bw_out',dest='bw_out',type=bool,default=True,help='whether output iamge convert to binary image')
parser.add_argument('--bw_thread',dest='bw_thread',type=int,default=80,help='the threadshold of binaray')
...
parser.add_argument('--blind_dir', dest='blind_dir', default='./blind', help='blind test sample are saved here')
...
def main(_):
    ...
    if not os.path.exists(args.blind_dir):
        os.mkdir(args.blind_dir)
