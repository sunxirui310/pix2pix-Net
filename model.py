...
class pix2pix(object):
    ...
    def test(self, args):
        ...
        # sort testing input
        n = [str(i) for i in map(lambda x: x.split('\\')[-1].split('.jpg')[0], sample_files)]
        ...
        for i, sample_image in enumerate(sample_images):
            ...
            #tempImage=Image.fromarray((samples+1)/2,mode='L')
            save_images(samples, [self.batch_size, 1],
                        './{}/test_{:04d}.png'.format(args.test_dir, idx))

    def blind(self, args):
        """Test pix2pix,batchsize设置为1时有效"""
        init_op = tf.global_variables_initializer()
        self.sess.run(init_op)

        sample_files = glob('./datasets/{}/blind/*.jpg'.format(self.dataset_name))
        ...
        # load testing input
        print("Loading blind testing images ...")
        # preprocess input image to the right format
        ...
        #add record image name and resize input image
        ...
        for i, sample_image in enumerate(sample_images):
            ...
            #image resize to normal size
            if args.bw_out:
                print(sample_size[i])
                samples_bw =  resize_back(samples[0,:,:,:],sample_size[i][0],sample_size[i][1],args.bw_out,args.bw_thread)            
                scipy.misc.imsave('./{}/{}.jpg'.format(args.blind_dir, n[i]),samples_bw)
